...
class Initialize(commands.Cog):
    ...
    async def on_ready(self):
        print(
            f"{Bcolors.FAIL}{self.bot.user.name} has connected to Discord{Bcolors.ENDC}"
        )
    ...
    async def get_guilds(self):
        ...
        for guild in self.bot.guilds:
            if guild.id not in df.index:
                dead_role = await self.get_role(guild)
                if dead_role is None:
                    role = await self.bot.create_new_role(guild)
                    dead_role = 0 if role is None else role.id
                else:
                    dead_role = dead_role.id
                df.loc[guild.id] = [guild.name, dead_role, guild.preferred_locale]
            else:
                dead_role = await self.get_role(guild, df.at[guild.id, "dead"])
                if dead_role is None:
                    role = await self.bot.create_new_role(guild)
    ...
    async def get_role(self, guild, role_id=-1):
        return (
            discord.utils.get(guild.roles, name=Enums.Dead)
            if role_id == -1
            else guild.get_role(role_id)
        )
