...
class BasicCommands(commands.Cog):
    ...
    async def join(self, ctx):
        ...
        embed.set_thumbnail(url='https://media.discordapp.net/attachments/603600328117583874/615150515210420249/primal_aspid_king.gif')
        try:
    ...
    async def code(self, ctx):
        ...
        embed.set_image(url='https://media.discordapp.net/attachments/614108079545647105/629782304738377738/3032408cf9e547dc.png')
        await ctx.send(embed=embed)
    ...
    async def batya(self, ctx):
        embed = discord.Embed(color=discord.Colour.dark_green())
        embed.set_image(url="https://media.discordapp.net/attachments/614108079545647105/1015285820669833358/unknown.png")
        await ctx.send(embed=embed)
    ...
    @commands.command(name='message', help='Post a message as a server itself', pass_context=True)
    ...
    async def server(self, ctx):
        lang = self.bot.get_languages(ctx.guild)
        await ctx.message.delete()
        text = lang['server_1'] + ctx.guild.name + lang['server_2'] + f"<t:{int(ctx.guild.created_at.timestamp())}:R>\n\n" + \
                lang['server_3'] + str(len(ctx.guild.members)) + lang['server_4'] + str(len(ctx.guild.roles)) + \
                lang['server_5'] + str(len(ctx.guild.channels)) + lang['server_6']
        embed = discord.Embed(
    ...
    async def techo(self, ctx, _time= ' ', *, arg= ''):
        try:
            ...
            await ctx.send(f"Command must be in format `{self.bot.config['BOT_PREFIX']}techo <0-600> text`")
            return
